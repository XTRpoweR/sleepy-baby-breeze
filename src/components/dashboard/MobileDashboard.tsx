  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-pink-50 via-purple-50/60 to-indigo-50 overflow-x-hidden">
      ...
      <div aria-hidden="true" className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute top-0 left-0 w-80 h-80 rounded-full bg-pink-300/40 blur-2xl" />
        <div className="absolute top-40 right-0 w-80 h-80 rounded-full bg-purple-300/40 blur-2xl" />
        <div className="absolute bottom-0 left-0 w-80 h-80 rounded-full bg-indigo-300/30 blur-2xl" />
      ...
      <div
        className="relative px-4 pt-6 pb-24"
        onTouchStart={(e) => {
      ...
      >
          ...
          {!isNewUser ? (
            <div className="mb-8">
              ...
              {!isPremium && profiles.length >= 1 && (
                <div className="bg-gradient-to-r from-pink-50 to-purple-50 border border-purple-200/60 rounded-2xl p-3 mt-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="text-sm font-bold text-gray-900">Get Premium</h3>
                    ...
                    <p className="text-[11px] text-gray-600 truncate leading-snug">
                      Unlimited · sharing · reports
                    </p>
                  </div>
                  <Button
                    size="sm"
                    onClick={() => navigate('/subscription')}
                    className="flex-shrink-0 rounded-full font-bold bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 hover:from-pink-600 hover:via-purple-600 hover:to-indigo-600 text-white border-0 shadow-md shadow-purple-500/30 px-3 h-9"
                  >
                    Try
        ...
        <div className="space-y-4 mb-8">
          ...
          <button
            ...
            className="relative w-full text-left bg-white/85 border border-white/90 rounded-3xl p-5 transition-transform active:scale-[0.98] group"
            style={{
              boxShadow: '0 10px 30px -10px rgba(236, 72, 153, 0.18), 0 4px 12px -4px rgba(168, 85, 247, 0.12), inset 0 1px 0 0 rgba(255, 255, 255, 0.9)',
            }}
          >
            ...
            <div className="flex items-center space-x-4">
              <div className="bg-gradient-to-br from-pink-500 to-rose-500 rounded-2xl p-3.5 shadow-lg shadow-pink-500/30 flex-shrink-0">
                <Activity className="h-7 w-7 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-gray-900 mb-0.5">{t('dashboard.trackActivities')}</h3>
          ...
          <button
            ...
            className="relative w-full text-left bg-white/85 border border-white/90 rounded-3xl p-5 transition-transform active:scale-[0.98] group"
            style={{
              boxShadow: '0 10px 30px -10px rgba(99, 102, 241, 0.18), 0 4px 12px -4px rgba(168, 85, 247, 0.12), inset 0 1px 0 0 rgba(255, 255, 255, 0.9)',
            }}
          >
            <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white to-transparent rounded-t-3xl" />
            <div className="flex items-center space-x-4">
              <div className="bg-gradient-to-br from-indigo-500 to-purple-500 rounded-2xl p-3.5 shadow-lg shadow-indigo-500/30 flex-shrink-0">
                <Moon className="h-7 w-7 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-gray-900 mb-0.5">{t('dashboard.sleepSchedule')}</h3>
        ...
        <div className="mb-8" style={{ contentVisibility: 'auto', containIntrinsicSize: '800px' }}>
        ...
        {!isNewUser && (
          <div className="mb-8" style={{ contentVisibility: 'auto', containIntrinsicSize: '400px' }}>
            <h3 className="text-xl font-bold text-foreground mb-4">Weekly Insights</h3>
            <div className="grid grid-cols-2 gap-4">
              <MobileStatGlassCard isLoading={statsLoading} icon={<Moon className="h-5 w-5 text-white" />} value={dashboardStats.weeklyAverageSleep} label="Avg Sleep" gradient="from-indigo-500 to-purple-500" />
              <MobileStatGlassCard isLoading={statsLoading} icon={<Milk className="h-5 w-5 text-white" />} value={dashboardStats.weeklyFeedings} label="Feedings" gradient="from-emerald-500 to-teal-500" />
              <MobileStatGlassCard isLoading={statsLoading} icon={<Shield className="h-5 w-5 text-white" />} value={dashboardStats.weeklyDiaperChanges} label="Diapers" gradient="from-amber-500 to-orange-500" />
              <MobileStatGlassCard isLoading={statsLoading} icon={<Activity className="h-5 w-5 text-white" />} value={stats?.todayActivities || 0} label="Today" gradient="from-pink-500 to-fuchsia-500" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <Button
          onClick={onTrackActivity}
          className="w-16 h-16 rounded-full bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 hover:from-pink-600 hover:via-purple-600 hover:to-indigo-600 shadow-2xl shadow-purple-500/40 transform transition-all duration-300 hover:scale-110 active:scale-95"
          size="icon"
        >
...
const MobileFeatureGlassCard = memo(function MobileFeatureGlassCard({
  onClick,
  icon,
  title,
  subtitle,
  ...
  return (
    <button
      onClick={onClick}
      className={`relative text-left bg-white/85 border border-white/90 rounded-3xl p-4 transition-transform active:scale-[0.97] ${wide ? 'col-span-2' : ''}`}
      style={{
        boxShadow: `0 8px 24px -8px ${shadowColor}, inset 0 1px 0 0 rgba(255, 255, 255, 0.9)`,
      }}
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white to-transparent rounded-t-3xl" />
      {isPremium && (
        <span className="absolute top-2.5 right-2.5 inline-flex items-center justify-center h-5 w-5 rounded-full bg-gradient-to-br from-amber-300 to-amber-500 shadow-md ring-2 ring-white">
          <Sparkles className="h-2.5 w-2.5 text-white" strokeWidth={3} />
        </span>
      )}
      <div className={wide ? 'flex items-center space-x-3' : 'text-center'}>
        <div className={`bg-gradient-to-br ${gradient} rounded-2xl w-12 h-12 flex items-center justify-center shadow-md flex-shrink-0 ${wide ? '' : 'mx-auto mb-3'}`}>
          {icon}
        </div>
...
const MobileStatGlassCard = memo(function MobileStatGlassCard({
  isLoading,
  icon,
  value,
  label,
  ...
  return (
    <div
      className="relative bg-white/85 border border-white/90 rounded-3xl p-4 text-center"
      style={{
    ...
    >
      ...
      {isLoading ? (
        <>
          <Skeleton className="w-10 h-10 rounded-2xl mx-auto mb-3" />
          <Skeleton className="h-5 w-16 mx-auto mb-1" />
          <Skeleton className="h-3 w-12 mx-auto" />
        </>
      ) : (
        <>
          <div className={`bg-gradient-to-br ${gradient} rounded-2xl w-10 h-10 flex items-center justify-center mx-auto mb-3 shadow-md`}>
            {icon}
          </div>
