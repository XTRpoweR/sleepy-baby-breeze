            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-semibold">
                  {invoices.length > 0 ? formatCurrency(invoices.reduce((sum, inv) => sum + inv.amount_paid, 0), invoices[0]?.currency || 'usd') : '$0.00'}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={handleRefreshInvoices} className="flex items-center space-x-2">
        ...
        <CardContent>
          {invoices.length === 0 ? <div className="text-center py-12">
              ...
              {invoices.map(invoice => <div key={invoice.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(invoice.invoice_status)}`}>
                          {invoice.invoice_status.toUpperCase()}
                        </span>
                        {isInvoicePDF(invoice) && <span className="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-600">
                            PDF
                          </span>}
                      </div>
                    ...
                    <div className="flex items-center space-x-2">
                      {invoice.invoice_pdf_url ? <>
                          <Button variant="outline" size="sm" onClick={() => handleViewInvoice(invoice)} className="flex items-center space-x-1">
                            <Eye className="h-4 w-4" />
                            <span>View</span>
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => downloadInvoice(invoice)} className="flex items-center space-x-1">
                            <Download className="h-4 w-4" />
                            <span>Download</span>
                          </Button>
                          {!isInvoicePDF(invoice) && <Button variant="outline" size="sm" onClick={() => handleGeneratePDF(invoice)} className="flex items-center space-x-1 text-blue-600">
                          ...
                          <Button variant="outline" size="sm" onClick={() => handleGeneratePDF(invoice)} className="flex items-center space-x-1">
                            <FileText className="h-4 w-4" />
          ...
          {invoices.length > 0 && <div className="mt-6 pt-4 border-t text-center">
              <p className="text-sm text-gray-600">
                Need help with an invoice? Contact{' '}
                <a href="mailto:support@sleepybaby.com" className="text-blue-600 hover:text-blue-800">support@sleepybabyy.com</a>
              </p>
            </div>}
        </CardContent>
      </Card>
