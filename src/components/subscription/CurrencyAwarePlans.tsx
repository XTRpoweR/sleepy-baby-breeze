...
import { Check, Crown, Baby, Clock, BarChart3, Users, Calendar, Download, Sparkles } from 'lucide-react';
...
const CURRENCY_PRICING: Record<string, CurrencyPricing> = {
  USD: { symbol: '$', amount: '9.99', originalAmount: '14.99' },
  EUR: { symbol: '€', amount: '9.99', originalAmount: '14.99' },
  SEK: { symbol: 'kr', amount: '109', originalAmount: '149' },
  GBP: { symbol: '£', amount: '8.99', originalAmount: '12.99' }
};

const detectUserCurrency = (): string => {
  // Try to detect user's currency based on browser language and locale
  const language = navigator.language;
  
  if (language.includes('sv') || language.includes('SE')) return 'SEK';
  if (language.includes('en-GB') || language.includes('GB')) return 'GBP';
  if (language.includes('de') || language.includes('fr') || language.includes('it') || language.includes('es')) return 'EUR';
  
  // Default to EUR for better payment method coverage
  return 'EUR';
};

export const CurrencyAwarePlans = () => {
  const {
    subscriptionTier,
    createCheckout,
    upgrading,
  ...
  const [pricing, setPricing] = useState<CurrencyPricing>(CURRENCY_PRICING.EUR);

  useEffect(() => {
    const detectedCurrency = detectUserCurrency();
    setCurrency(detectedCurrency);
    setPricing(CURRENCY_PRICING[detectedCurrency] || CURRENCY_PRICING.EUR);
  }, []);
  ...
  const basicFeatures = [
    ...
    {
      icon: Clock,
      text: "Current day activity tracking",
      available: true
    },
    {
      icon: BarChart3,
      text: "Basic sleep reports",
      available: true
    },
    {
      icon: Calendar,
  ...
  const premiumFeatures = [
    ...
    {
      icon: Clock,
      text: "Extended activity history",
      available: true
    },
    {
      icon: BarChart3,
    ...
    {
      icon: Users,
      text: "Family sharing & collaboration",
      available: true
    },
    {
      icon: Calendar,
      text: "AI-powered sleep optimization",
      available: true
    },
    {
      icon: Download,
      ...
      <Card className={`relative transition-all duration-300 hover:shadow-xl ${user && subscriptionTier === 'basic' ? 'ring-2 ring-blue-500' : 'hover:-translate-y-1'}`}>
        <CardHeader className="text-center pb-6 lg:pb-8">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            <CardTitle className="text-lg sm:text-xl">SleepyBabyy Basic</CardTitle>
      ...
      <Card className={`relative transition-all duration-300 hover:shadow-xl ${user && subscriptionTier === 'premium' ? 'ring-2 ring-orange-500' : 'ring-2 ring-orange-200 hover:-translate-y-1'}`}>
        ...
        <CardHeader className="text-center pb-6 lg:pb-8">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            <CardTitle className="text-lg sm:text-xl">SleepyBabyy Premium</CardTitle>
          ...
          <div className="flex items-center justify-center space-x-2 mb-1">
            <span className="text-lg sm:text-xl text-gray-500 line-through font-medium">
              {pricing.symbol}{pricing.originalAmount}
            </span>
          ...
          <p className="text-red-600 text-xs sm:text-sm font-medium">
            Save {pricing.symbol}{pricing.originalAmount && pricing.amount ? 
              (parseFloat(pricing.originalAmount) - parseFloat(pricing.amount)).toFixed(2) : 
              '5.00'} per month!
          </p>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Pricing in {currency} • Multiple payment methods available
          </p>
        </CardHeader>
        <CardContent className="space-y-4 lg:space-y-6">
          <div className="space-y-3 lg:space-y-4">
          ...
          {(!user || !isPremium) && (
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-gray-500">• Klarna & other payment methods available</p>
            </div>
          )}
        </CardContent>
      </Card>
