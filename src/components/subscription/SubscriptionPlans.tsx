      ...
      <Card className={`relative transition-all duration-300 hover:shadow-xl ${user && subscriptionTier === 'premium' ? 'ring-2 ring-orange-500' : 'ring-2 ring-orange-200 hover:-translate-y-1'}`}>
        ...
        <CardHeader className="text-center pb-6 lg:pb-8">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            {user && subscriptionTier === 'premium' && !isTrial && (
        ...
        <CardContent className="space-y-4 lg:space-y-6">
          ...
          <Button 
            ...
            disabled={upgrading || (user && isPremium && !isTrial)}
          >
            {upgrading ? 'Processing...' : 
             user && isPremium && !isTrial ? 'Current Plan' : 
             user && isTrial ? `Trial (${trialDaysLeft} days left)` : 
             user ? 'Start Free Trial' : 'Start Free Trial'}
          </Button>
          {(!user || !isPremium || isTrial) && (
            <p className="text-xs text-center text-gray-500">
              {isTrial ? ' • 7-day free trial • Cancel anytime during trial' : ' • 7-day free trial • Cancel anytime'}
            </p>
          )}
        </CardContent>
      </Card>
