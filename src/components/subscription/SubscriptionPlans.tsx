      ...
      <Card className={`relative transition-all duration-300 hover:shadow-xl ${user && subscriptionTier === 'basic' ? 'ring-2 ring-blue-500' : 'hover:-translate-y-1'}`}>
        <CardHeader className="text-center pb-6 lg:pb-8">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            {user && subscriptionTier === 'basic' && <Badge variant="default" className="bg-blue-100 text-blue-800 text-xs">
                Current Plan
              </Badge>}
          </div>
        ...
        <CardContent className="space-y-4 lg:space-y-6">
          <div className="space-y-3 lg:space-y-4">
            {basicFeatures.map((feature, index) => <div key={index} className="flex items-center space-x-3">
          ...
          <Button className="w-full touch-target" variant={user && subscriptionTier === 'basic' ? 'default' : 'outline'} disabled={user && subscriptionTier === 'basic'} onClick={() => !user && navigate('/auth')}>
      ...
      <Card className={`relative transition-all duration-300 hover:shadow-xl ${user && subscriptionTier === 'premium' ? 'ring-2 ring-orange-500' : 'ring-2 ring-orange-200 hover:-translate-y-1'}`}>
        ...
        <CardHeader className="text-center pb-6 lg:pb-8">
          <div className="flex items-center justify-center space-x-2 mb-2">
            ...
            <CardTitle className="text-lg sm:text-xl text-red-600">SleepyBaby Premium</CardTitle>
            {user && subscriptionTier === 'premium' && <Badge variant="default" className="bg-orange-100 text-orange-800 text-xs">
                Current Plan
              </Badge>}
          </div>
          <div className="flex items-center justify-center space-x-1">
            <span className="text-3xl sm:text-4xl font-bold text-gray-900">$9.99
          </span>
            <span className="text-gray-600 font-medium text-sm sm:text-base">40% OFF</span>
          </div>
          <p className="text-sm my-[6px] px-[7px] py-px text-slate-600 text-center mx-[39px] sm:text-xl font-semibold">$14.99
Complete baby tracking solution</p>
        </CardHeader>
        <CardContent className="space-y-4 lg:space-y-6">
          <div className="space-y-3 lg:space-y-4">
            {premiumFeatures.map((feature, index) => <div key={index} className="flex items-center space-x-3">
                <div className="bg-green-100 rounded-full p-1">
                  <Check className="h-3 w-3 text-green-600 flex-shrink-0" />
                </div>
