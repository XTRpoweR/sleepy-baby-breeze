...
export const SleepScheduleReportDialog = ({ isOpen, onClose, schedule }: SleepScheduleReportDialogProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="fixed inset-0 z-50 bg-white w-full h-full p-4 overflow-y-auto sm:inset-auto sm:left-1/2 sm:top-1/2 sm:translate-x-[-50%] sm:translate-y-[-50%] sm:w-full sm:max-w-4xl sm:h-auto sm:max-h-[90vh] sm:rounded-lg sm:p-6">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <DialogHeader className="mb-6">
          <DialogTitle className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
