...
/**
 * Computes weekly dashboard statistics for a given baby profile.
 *
 * Pass the active baby/profile id explicitly so stats always stay in sync
 * with whatever profile the UI is showing.
 */
export const useDashboardStats = (profileId?: string) => {
  const resolvedProfileId = profileId || '';
  const { logs, loading } = useActivityLogs(resolvedProfileId);
  ...
  const stats = useMemo<DashboardStats>(() => {
    ...
    const daysSinceMonday = currentDay === 0 ? 6 : currentDay - 1; // Sunday = 0
    ...
    return finalStats;
  }, [logs, loading, profileId, resolvedProfileId]);

  return {
    stats,
    loading,
    hasActiveProfile: !!resolvedProfileId,
  };
};
